...
-(void)startDownLoad
{
    ...
    }else
    {
        ...
        self.musicDownLoadLab.text=self.placeholderBtnText?self.placeholderBtnText:[NSString stringWithFormat:@"Completed"];
...
{
    musicalText=self.placeholderText?self.placeholderText:@"Loading...";
    ...
    self.currentLoadLb = [[UILabel alloc] init];
    self.currentLoadLb.textAlignment = NSTextAlignmentCenter;
    self.currentLoadLb.textColor=[UIColor whiteColor];
    self.currentLoadLb.font=[UIFont fontWithName:@"Helvetica-Bold" size:12];
    self.currentLoadLb.text = @"Current: 0 /Total: 0";
    self.currentLoadLb.adjustsFontSizeToFitWidth=YES;
    [self addSubview:self.currentLoadLb];
    
    [self.currentLoadLb makeConstraints:^(MASConstraintMaker *make){
        make.top.equalTo(self.musicDownLoadLab).offset(15);
        make.centerX.equalTo(self);
        make.width.equalTo(@80);
        make.height.equalTo(@20);
    }];
    
    self.totalLoadLb = [[UILabel alloc] init];
    self.totalLoadLb.textAlignment = NSTextAlignmentCenter;
    self.totalLoadLb.textColor=[UIColor whiteColor];
    self.totalLoadLb.font=[UIFont fontWithName:@"Helvetica-Bold" size:12];
    self.totalLoadLb.adjustsFontSizeToFitWidth=YES;
    [self addSubview:self.totalLoadLb];
    
    [self.totalLoadLb makeConstraints:^(MASConstraintMaker *make){
        make.top.equalTo(self.musicDownLoadLab).offset(20);
        make.left.equalTo(self.totalLoadLb).offset(90);
        make.width.equalTo(@80);
        make.height.equalTo(@20);
    }];
}
